...
static void set_key(cf_chacha20_ctx *ctx, const uint8_t *key, size_t nkey)
{
  ...
  {
    case 16:
      pen_memcpy(ctx->key0, key, 16);
      pen_memcpy(ctx->key1, key, 16);
      ctx->constant = chacha20_tau;
      break;
    case 32:
      pen_memcpy(ctx->key0, key, 16);
      pen_memcpy(ctx->key1, key + 16, 16);
...
void cf_chacha20_init(cf_chacha20_ctx *ctx, const uint8_t *key, size_t nkey, const uint8_t nonce[8])
{
  ...
  pen_memset(ctx->nonce, 0, sizeof ctx->nonce);
  pen_memcpy(ctx->nonce + 8, nonce, 8);
  ctx->nblock = 0;
  ctx->ncounter = 8;
}
...
{
  pen_assert(ncounter > 0);
  set_key(ctx, key, nkey);
  pen_memcpy(ctx->nonce, nonce, sizeof ctx->nonce);
  ctx->nblock = 0;
