...
static void ghash_add_pad(ghash_ctx *ctx)
{
  ...
  pen_memset(ctx->buffer + ctx->buffer_used, 0, sizeof(ctx->buffer) - ctx->buffer_used);
...
{
  pen_assert(ctx->state == STATE_AAD);
  ctx->len_aad += n;
  ghash_add(ctx, buf, n);
}
...
static void ghash_add_cipher(ghash_ctx *ctx, const uint8_t *buf, size_t n)
{
  ...
  pen_assert(ctx->state == STATE_CIPHER);
  ctx->len_cipher += n;
  ghash_add(ctx, buf, n);
}
                         ...
                         const uint8_t *nonce, size_t nnonce)
{
  ...
  if (nnonce == 12)
  {
    pen_memcpy(gcmctx->Y0, nonce, nnonce);
  ...
  /* Produce ciphertext */
  pen_memset(gcmctx->e_Y0, 0, sizeof(gcmctx->e_Y0));
