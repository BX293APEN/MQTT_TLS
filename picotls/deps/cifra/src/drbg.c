                             ...
                             void *out, size_t nout)
{
  ...
  pen_assert(!cf_hash_drbg_sha256_needs_reseed(ctx));

  hash_process_addnl(&cf_sha256, addnl, naddnl, ctx->V, sizeof ctx->V);
  pen_assert(sizeof data == sizeof ctx->V);
  pen_memcpy(data, ctx->V, sizeof ctx->V);
                                ...
                                const void *addnl, size_t naddnl)
{
  ...
  pen_memcpy(ctx->C, ctx->V, sizeof ctx->C);
  hash_df(&cf_sha256,
          &one, sizeof one,
          ctx->C, sizeof ctx->C,
          entropy, nentropy,
                       ...
                       const void *persn, size_t npersn)
{
  ...
  pen_assert(hash->hashsz <= CF_MAXHASH);

  /* 2. Key = 0x00 00 ... 00
   * 3. V = 0x01 01 ... 01 */
  uint8_t initial_key[CF_MAXHASH];
  pen_memset(initial_key, 0x00, hash->hashsz);
  pen_memset(ctx->V, 0x01, hash->hashsz);
                               ...
                               void *out, size_t nout)
{
  ...
  pen_assert(!cf_hmac_drbg_needs_reseed(ctx));
