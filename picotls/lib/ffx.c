...
#include <penlib.h>
/* penlib.h が unistd.h (POSIX) / wincompat.h 相当 (_WINDOWS) を集約済み */
#include "picotls.h"
                          ...
                          const void *key)
{
    ...
    pen_assert(len <= 32 && len >= 2);
    pen_assert(ctx->super.do_dispose == NULL);
    pen_assert(ctx->super.do_init == NULL);
    pen_assert(ctx->super.do_transform == NULL);
    pen_assert(ctx->super.algo == NULL || algo->key_size == ctx->super.algo->key_size);
    pen_assert(ctx->super.algo == NULL || algo->iv_size == ctx->super.algo->iv_size);
    pen_assert(ctx->super.algo == NULL || ctx->super.algo->block_size == len);
    pen_assert(algo->iv_size == 16);
...
static void ffx_dispose(ptls_cipher_context_t *_ctx)
{
    ...
    pen_assert(ctx->super.do_dispose == ffx_dispose);
...
{
    ptls_cipher_context_t *ctx = (ptls_cipher_context_t *)pen_malloc(sizeof(ptls_ffx_context_t));

    if (ctx != NULL) {
        pen_memset(ctx, 0, sizeof(ptls_ffx_context_t));
...
static void ffx_encrypt(ptls_cipher_context_t *_ctx, void *output, const void *input, size_t len)
{
    ...
    pen_assert(ctx->super.do_transform == ffx_encrypt);

    /* len must match context definition */
    pen_assert(len == ctx->byte_length);
    if (len != ctx->byte_length) {
        pen_memset(output, 0, len); /* so that we do not leak anything in production mode */
        return;
    }

    /* Split the input in two halves */
    pen_memcpy(left, input, ctx->nb_left);
    pen_memcpy(right, ((uint8_t *)input) + ctx->nb_left, ctx->nb_right);
    pen_memset(left + ctx->nb_left, 0, 16 - ctx->nb_left);
    pen_memset(right + ctx->nb_right, 0, 16 - ctx->nb_right);
    ...
    pen_memcpy(output, left, ctx->nb_left);

    right[ctx->nb_right - 1] &= ctx->mask_last_byte;
    right[ctx->nb_right - 1] |= (last_byte & ~ctx->mask_last_byte);

    pen_memcpy(((uint8_t *)output) + ctx->nb_left, right, ctx->nb_right);
...
{
    ptls_ffx_context_t *ctx = (ptls_ffx_context_t *)_ctx;
    pen_assert(ctx->super.do_init == ffx_init);
    pen_memcpy(ctx->tweaks, iv, 16);
}
